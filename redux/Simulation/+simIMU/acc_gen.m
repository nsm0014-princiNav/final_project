...
elseif (isfield(ref, 'vel'))
    ...
    acc_ned = simIMU.my_sgolayfilt(acc_raw);
    acc_b = conversion.acc_nav2body(acc_ned, ref.DCMnb_m);
...
g_n = -simIMU.gravity(ref.lat, ref.h);              % Accelerometer in Z axis senses an 
                                                % acceleration of 1.0 G straight up
cor_n = simIMU.coriolis(ref.lat, ref.vel, ref.h);
...
ab_sta = simIMU.noise_b_sta (imu.ab_sta, N);
...
dt = 1/ref.freq; 
ab_dyn = simIMU.noise_b_dyn (imu.ab_corr, imu.ab_dyn, dt, M);

% -------------------------------------------------------------------------
% Simulation of rate random walk

acc_rrw = simIMU.noise_rrw (imu.vrrw, dt, M);
