...
for i = 1:N
    ...
    omega_ie_n = simIMU.earth_rate(ref.lat(i));
    omega_en_n = simIMU.transport_rate(ref.lat(i), ref.vel(i,1), ref.vel(i,2), ref.h(i));
    om_ie_n = conversion.skewm_inv(omega_ie_n);
    om_en_n = conversion.skewm_inv(omega_en_n);
...
gb_sta = simIMU.noise_b_sta (imu.gb_sta, N);
...
dt = 1/ref.freq; 
gb_dyn = simIMU.noise_b_dyn (imu.gb_corr, imu.gb_dyn, dt, M);

% -------------------------------------------------------------------------
% Simulation of rate random walk

gyro_rrw = simIMU.noise_rrw (imu.arrw, dt, M);
