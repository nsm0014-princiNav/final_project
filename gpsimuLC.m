function LC_Sol = gpsimuLC(static_data,data)
%% Defining Inputs
...
% GPS - TURN CASE
GPS.t = data.novatel_local.gpsTimeTagged.gpsSeconds';                  % t: Gx1 time vector (seconds).
GPS.lat = deg2rad(data.novatel_local.odom.positionLla(1,:)');          % lat: Gx1 latitude (radians).
GPS.lon = deg2rad(data.novatel_local.odom.positionLla(2,:)');          % lon: Gx1 longitude (radians).
GPS.h = data.novatel_local.odom.positionLla(3,:)';                     % h: Gx1 altitude (m).
GPS.vel = data.novatel_local.odom.velocityNed';                        % vel: Gx3 NED velocities (m/s)
